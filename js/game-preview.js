...
(function (global) {
    ...
    let settings = { stack: true, brainrot: false, pistolHp: 3 };
    ...
    function parsePreviewOptions() {
        ...
        const bots = Math.min(3, Math.max(1, parseInt(params.get('bots') || '2', 10) || 2));
        botIds = Array.from({ length: bots }, (_, i) => `bot-${i + 1}`);
        settings = {
            stack: params.get('stack') !== '0',
            brainrot: params.get('brainrot') === '1',
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (bar) {
            ...
            if (label) {
                label.textContent = `Test locale · ${botIds.length} bot · Stack ${settings.stack ? 'ON' : 'OFF'}`;
            }
        }
    });
